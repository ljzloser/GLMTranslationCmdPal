...
internal partial class GLMTranslationPage : DynamicListPage, IDisposable, IFallbackHandler
{
    ...
    private void AddResponseItems(string responseContent)
    {
        ...
        Dictionary<string, string> dic = JsonSerializer.Deserialize<Dictionary<string, string>>(content)
            ?? new Dictionary<string, string> { ["Error"] = content };
    ...
    {
        // 提示词模板
        string temp = Resources.SystemText;
        ...
        var system_text = string.Format(System.Globalization.CultureInfo.InvariantCulture, temp,args);
